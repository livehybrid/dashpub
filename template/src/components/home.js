...
export const CardLayout = dynamic(
  () => import("@splunk/react-ui/CardLayout"),
  { ssr: false, }
);

export const CardDefault = dynamic(
  () => import("@splunk/react-ui/Card"),
  { ssr: false, }
);

export const CardHeader = dynamic(
  () => import("@splunk/react-ui/Card").then((lib) => lib.Header),
  { ssr: false, }
);

export const CardBody = dynamic(
  () => import("@splunk/react-ui/Card").then((lib) => lib.Body),
  { ssr: false, }
);

export const CardFooter = dynamic(
  () => import("@splunk/react-ui/Card").then((lib) => lib.Footer),
  { ssr: false, }
);
...
export const Chip = dynamic(
  () => import("@splunk/react-ui/Chip"),
  { ssr: false, }
);

export const Button = dynamic(
  () => import("@splunk/react-ui/Button"),
  { ssr: false, }
);
...
class AllTags extends Component {
    ...
    render() {
        ...
        font-weight: bold;
    `
        return (<div><TagTitle>Tags:</TagTitle>
            <Button selected={selectedTag===""} icon={<Tag className={`ba bi-tag mr-2 tag-all`} />} key={"all"} onClick={() => tagClick("")} className={"badge badge-pill text-dark m-2"}>
                All
            </Button>
            {
                uniqueTags.filter(tag => tag!=="hidden").map((tag) => {
                    const colorClass = selectedTag===tag ? "secondary" : "outline-secondary"
                    return <Button selected={selectedTag===tag} icon={<Tag className={`ba bi-tag mr-2 tag-{tag}`} />} key={tag} onClick={() => tagClick(tag)} variant={colorClass}
...
class Home extends Component {
	...
	const renderTags = (k) => {
			...
			return(
                                ...
                                <TagContainer>
                                    {dashboardManifest[k]['tags'].map((tag) => (
                                        <Chip className="m-1"  key={tag} icon={<Tag />}>{tag}</Chip>
                                    ))}
	    ...
	    if (INSERT_SCREENSHOTS) {
                ...
                return(<Card.Body><CardTitle>{dashboardManifest[k]['title']}</CardTitle><CardSubTitle>{dashboardManifest[k]['description']}</CardSubTitle></Card.Body>)
